	...
	public class ClassObjPool<T> : ClassObjPoolBase where T : PooledClassObject, new()
	{
		...
		public static T Get()
		{
			...
			if (ClassObjPool<T>.instance.pool.Count > 0)
			{
                ...
                ClassObjPool<T>.instance.spawns.Add(t);
                t.OnUse();
				return t;
			}
            ...
            ClassObjPool<T>.instance.spawns.Add(t2);
            t2.OnUse();
            return t2;
		}
        ...
        {
            ClassObjPool<T> pool = ClassObjPool<T>.instance;

            for (int i = pool.spawns.Count - 1; i >= 0; i --)
            {
                ((PooledClassObject)pool.spawns[i]).Release();
            }
        }
